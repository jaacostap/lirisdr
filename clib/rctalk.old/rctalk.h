
/*----------------------------------------------------------------------------*/
/*                                                                            */
/*                     Instituto de Astrofisica de Canarias                   */
/*                             Software Department                            */
/*                                                                            */
/*                               Proyecto LIRIS                               */
/*----------------------------------------------------------------------------*/

/*- start of module header information -----------------------------------------

COMPONENT:

   rctalk.h


HISTORY:

   Version  Date    Author    Description

   1.1              Jose Acosta    Correct row cross-talk


TYPE:

   C include file


PURPOSE:

   Include file for RCTALK package 


PORTABILITY:

   ANSI-C


DESCRIPTION:



--- End of module header information -----------------------------------------*/

/* Currently (06/08/00) floats have been selected */

/* Default quadrant offset parameters */
#define ACINTRA    1.43e-5
#define ACINTER    1.43e-5

/*#define or undef CORROW_DEBUG*/
#define RCTALK_DEBUG

/*- Feature test switches ----------------------------------------------------*/
/*- Headers required by this header ------------------------------------------*/
/*- Exported Macros ----------------------------------------------------------*/



/*- Exported typedefs, structures and unions ---------------------------------*/
/*- Exported data ------------------------------------------------------------*/
/*- Exported functions  ------------------------------------------------------*/

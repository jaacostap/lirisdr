...
begin
  ...
  real      skymidpt
  int       dimx,dimy,xmin,xmax,ymin,ymax,dbx1,dby1
  string    statsec
  ...
  while (fscan(list1, fname) != EOF)
    ...
    if (verbose) print "substracting sky to "//lfilename.reference//" ..."
      if (scale != "none")
       ...
       imgets(sky_aux,"i_naxis1")
       dimx =  int(imgets.value)
       imgets(sky_aux,"i_naxis2")
       dimy =  int(imgets.value)
       dbx1 = int(dimx * (1.-liskycpars.statsecx)/2.)
       dby1 = int(dimy * (1.-liskycpars.statsecy)/2.)
       xmin = dbx1 
       xmax = dimx - dbx1
       ymin = dby1
       ymax = dimy - dby1
       statsec = "["//(xmin)//":"//(xmax)//","//(ymin)//":"//(ymax)//"]"  
       print ("Statsec ",statsec)
       imstatistic(sky_aux//statsec,
                     fields=fld,format=no,binwidth=0.001) | scan(scale1)
       ...
       imstatistics(sky_aux//statsec,fields="midpt",format=no,binwidth=0.001) |
	 scan(skymidpt) 
       ...
       imstatistics(sky//statsec,fields="midpt",format=no,binwidth=0.001) |
	 scan(skymidpt)	 
       imarith( operand1 = fname,
                    op = "-",
              operand2 = sky,
       ...
       if (zero == "median")	
         fld="midpt" 
       imstatistics(sky_aux//statsec,fields=fld,format=no,binwidth=0.001) | 
         scan(scale1)

...
string  ftrim1,ofltpc, ofltsec,fsecext, iflt, trlist, imosec
...
#  
  ...
  while (fscan(list1,iflt) != EOF)
	...
	ltrimpol(iflt,ofltpc,ftrim1,outlist=trlist,ismask=no)
  ...
  if (isfltdk != "") 
   ...
   for (isec=1; isec<=4; isec= isec+1)
	...
	case 4: 
        ...
        imstatistics(flattmp//ofltsec,fields="midpt",format=no) | scan(md)
        imarith(flattmp//ofltsec,"/",md,nflattmp//ofltsec)
     }	

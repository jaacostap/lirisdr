...
begin  
  ...
  string	skyli,inli,outli,scaleli,subli,objmask,statsec
  string	logcomb
  ...
  inli = mktemp(tmpdir//"_ldithskyinli")
  ...
  outli = mktemp(tmpdir//"_ldithskyoutli")
  ...
  # We verify if some output image already exist
  list1 = inli
  subli = mktemp(tmpdir//"_lsubli")
  While (fscan(list1,fname) !=EOF)
    {
    #lfileaccess(fname,verbose=no)
    #if (lfileaccess.exist)
    #  {
    #    imsubsky = mktemp(tmpdir//"_lsubimsubsky")
  ...
  if (insky != "") 
    ...
    skyli = mktemp(tmpdir//"_ldithskyskyli")
...
##########################################################################
  ...
  if ( insky == "" )
    ...
    print ("acabo de entrar en insky vacio")
	...
	{ 
      ...
      lmksky( input = "@"//osfn(skyli),
	   ...
	   while (fscan(list1,onesky) != EOF)
	   {
	     ...
	     if (outmask == "") 
                 objmask = mktemp(tmpdir//"ldithskyslfomsk")//".pl"
	     else
	       {
	         lfilename(onesky)
	     ...
	     print ("creating mask file ",objmask)	 	 
	      lstatist(sky_aux,nsigrej=5.,maxiter=5,addheader=no,
		 ...
		 convolve="block 3 3",hsigma=6,hdetect=yes,ldetect=no,
...
#    {
  ...
  print ("sky ",sky)
  imstatistics(sky,binwidth=0.01)
  lsubscalesky("@"//inli,"@"//subli,sky,scale=liskycpars.scale,
     zero=liskycpars.zero)
  
  imstatistics("@"//subli,binwidth=0.01)
...
############################################################################
  ...
  if (maskobj && insky == "")
      ...
      while (fscan(list1, fname) != EOF)
	{
	 ...
	 if (outmask == "") 
             objmask = mktemp(tmpdir//"_ldthskomsk")//".pl"
	 else 
	   {
	     lfilename(fname)
	     #objmask = lfilename.path//outmask//lfilename.root//".pl"
...
#               niterate = lcvgradpars.niter)
	 ...
	 lstatist(fname1//statsec,nsigrej=5.,maxiter=5,addheader=no,
		 lower=INDEF,upper=INDEF,oneparam="all",verbose=no,print=no)
	 sg = (lstatis.median-lstatis.quart1)*1.4	
         print ("lsubdithsky - ",fname1//statsec," sg ",sg)  
         print ("objmasks ",objmask)
...
############################################################################
  ...
  imdel ("_lsubimsubsky*",verify=no, >>&"/dev/null")

# lspskynod - generate combined substracted sky image from spectral images 
# Author : Miguel Charcos (mcharcos@ll.iac.es)
# Version: 25. Nov. 2003
##############################################################################
procedure lspskynod(input)

string	input		{prompt="Name of input spectral image list"}
string	output		{"",prompt="Output substracted image prefix name"}
string  outlst		{"",prompt="Output list file containing substracted images"}
string  outlsta	        {"",prompt="Output list file containing substracted images at pos A"}
string  outlstb  	{"",prompt="Output list file containing substracted images at pos B"}
...
begin   
  ...
  string	skyli,inli,inli0,outli,outlia,outlib,ofli,imAli,imBli,ino
  string	logf
  string	imAstdout,imBstdout,statsec
  ...
  inli0 = mktemp(tmpdir//"ldithskyinli")
...
#  page(inli)
  ...
  if (access(outlia))
    ...
    beep
    print("")
    print("ERROR: operation would overwrite output image list file ",outlia)
    print("lspskynod aborted")
    print("")
    beep
    bye
  ...
  if (access(outlib))
    ...
    beep
    print("")
    print("ERROR: operation would overwrite output image list file ",outlib)
    print("lspskynod aborted")
    print("")
    beep
    bye
  ...
  if ( isky != "" )
	...
	if (outli != "") {
	     print (imsubsky, >>outli)
	     print (imsubskyb, >>outli)
	   } 
	if (outlia != "") {
	     print (imsubsky, >>outlia)
	   } 
	if (outlib != "") {
	     print (imsubskyb, >>outlib)
	   } 
